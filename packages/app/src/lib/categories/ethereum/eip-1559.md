...
We can use box plots to visualize the distribution of `priority_fee` in the past 5 blocks. The spread of the box is huge,
with a lot of outliers in most blocks. This means that there is a lot of room for setting a "good" `priority_fee`.

<BlockInput />
<PriorityFee />
<PendingTransactions />

We can also look at the past 1000 blocks to see how the distribution of `gas` has changed over time.
<GasFee/>
